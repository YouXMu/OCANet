...
class Omni_Attention(nn.Module):
    ...
    def __init__(self,dim,feature_dim,bias):
        ...
        self.omni_attn = ResBlock(dim,dim,feature_dim,filter=True)
        self.kernel = nn.Sequential(
...
class Omni_Attention_shallow(nn.Module):
    ...
    def __init__(self,dim,feature_dim,bias):
        ...
        self.omni_attn_shallow = ResBlock_shallow(dim,dim,feature_dim,filter=True)
        self.kernel = nn.Sequential(
            nn.Linear(256,dim,bias=bias)
        )
...
class GatedEmb(nn.Module):
    def __init__(self, in_c=3, embed_dim=48, bias=False):
        ...
        self.gate_proj = nn.Conv2d(in_c, embed_dim*2, kernel_size=3,stride=1,padding=1,bias=bias)
...
class DIformer(nn.Module):
    def __init__(self, 
        ...
        self.latent = nn.Sequential(*[TransformerBlock(dim=int(dim*2**3),  ffn_expansion_factor=ffn_expansion_factor, feature_dim = features[3] ,bias=bias, LayerNorm_type=LayerNorm_type) for i in range(num_blocks[3])])
    ...
    def forward(self, inp_img, k_v):
        ...
        # out_dec_level1 = torch.sigmoid(out_dec_level1)
...
class CPEN(nn.Module):
    def __init__(self,n_feats = 64, n_encoder_res = 6):
        ...
        E1=[nn.Conv2d(112, n_feats, kernel_size=3, padding=1),
...
class OCANetS1(nn.Module):
    def __init__(self, 
        n_encoder_res=5,         
        inp_channels=4, 
        out_channels=3, 
        dim = 48,
        num_blocks = [3,5,6,6,1,1,1],
        num_refinement_blocks = 4,
        ...
        super(OCANetS1, self).__init__()

        # Generator
        # Dynamic Inpainting Transformer
        self.G = DIformer(
