...
class PSNRScore(PairwiseScore):
    def __init__(self):
        super().__init__()
        self.score = PSNR(255.0).eval()
        self.reset()
        self.tag = False

    def forward(self, pred_batch, target_batch,mask=None):
        pred_batch,target_batch = self.postprocess(pred_batch,target_batch)
        batch_values = self.score(target_batch,pred_batch)
        self.individual_values = np.hstack([
            self.individual_values, batch_values.detach().cpu().numpy()
        ])
        # print(self.individual_values)
        return batch_values

    def postprocess(self, pre,gt):
        pre = pre.clamp_(0, 1) * 255.0  # b c h w
        pre = pre.permute(0, 2, 3, 1)  # b h w c
