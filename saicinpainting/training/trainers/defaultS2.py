...
from saicinpainting.training.losses.feature_matching import feature_matching_loss, masked_l1_loss, KDLoss,l1_IPR_loss
from saicinpainting.training.modules.fake_fakes import FakeFakesGenerator
from saicinpainting.training.trainers.baseS1 import BaseInpaintingTrainingModule, make_multiscale_noise
from saicinpainting.utils import add_prefix_to_keys, get_ramp
from saicinpainting.training.trainers.baseS2 import SBaseInpaintingTrainingModule
...
class SDefaultInpaintingTrainingModule(SBaseInpaintingTrainingModule):
    ...
    def forward(self, batch):
        ...
        if self.training:
            _, batch['fea_S1'] = self.generatorS1.E(masked_img,img)  # 一个256维的token  一个装有256维的列表
            # batch['predicted_image'], batch['pred_deg_list'] = self.generator(masked_img,batch['fea_S1'][0])
            batch['predicted_image'], batch['IPR'] = self.generator(masked_img)
        else:
            batch['predicted_image'], _ = self.generator(masked_img)
    ...
    def generator_loss(self, batch):
        ...
        if self.training:
            # i=len(batch['pred_deg_list'])-1
            # S_fea=[batch['pred_deg_list'][i]]
            # l_kd, l_abs = KDLoss(batch['fea_S1'],S_fea,self.config.losses.kd.loss_weight,self.config.losses.kd.temperature)
            # total_loss  += l_abs
            # metrics = dict(gen_labs=l_abs)
            # metrics = dict(gen_labs=l_kd)
            ##############
            l1_ipr = l1_IPR_loss(batch['IPR'],batch['fea_S1'][0])

            total_loss += l1_ipr
            metrics = dict(gen_ipr=l1_ipr)
