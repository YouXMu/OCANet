...
from saicinpainting.utils import add_prefix_to_keys, average_dicts, set_requires_grad, flatten_dict, \
    get_has_ddp_rank, print_different_keys_loading

LOGGER = logging.getLogger(__name__)

def load_network(net, load_path, strict=True):
    """Load network.
    ...
    print_different_keys_loading(net,load_net)
...
class SBaseInpaintingTrainingModule(ptl.LightningModule):
    def __init__(self, config, use_ddp, *args,  predict_only=False, visualize_each_iters=100,
                 ...
                 average_generator_period=10, store_discr_outputs_for_vis=False, generatorS2_path=None, generatorS1_path=None,checkpoint = None,
                 **kwargs):
        super().__init__(*args, **kwargs)
        LOGGER.info('SBaseInpaintingTrainingModule init called')

        self.config = config

        self.generator = make_generator(config, **self.config.generator)  # 这里是S2
        self.generatorS1 = make_generator(config, **self.config.generatorT)  # 这里是S1
        if checkpoint and generatorS1_path:
            # self.generator = SBaseInpaintingTrainingModule.load_from_checkpoint(checkpoint_path=checkpoint)
            load_network(self.generatorS1,generatorS1_path,strict=False)
        else:
            if generatorS2_path:
